...
//         let status = "";
//         let liveUpdateMsg = liveUpdate; // Preserve liveUpdate unless needed
...
//               liveUpdateMsg = "Waiting for your move submission";
//             } else {
//               status = game.player2 === ethers.ZeroAddress
//                 ? "Waiting for Player 2 to join"
//                 : "Waiting for Player 2's move";
//               liveUpdateMsg = game.player2 === ethers.ZeroAddress
//                 ? "Player 1 move submitted successfully, waiting for Player 2 to join"
//                 : "Player 2 has joined, waiting for their move";
//             }
//           } else if (isPlayer2) {
//             status = game.move2 === 0
//               ? "Lobby full, submit your move"
//               : "Waiting for resolution";
//             liveUpdateMsg = game.move2 === 0
//               ? "You joined as Player 2, submit your move"
//               : "Both players have submitted their moves, waiting for game resolution";
//           } else {
//             status = "Game started, join to play";
//             liveUpdateMsg = game.move1 === 0
//               ? "Player 1 is in this game, waiting for you to join"
//               : "Player 1 has submitted their move, join to play";
//           }
//         } else if (state === 1) {
//           status = "All moves submitted, awaiting resolution";
//           liveUpdateMsg = "Both players have submitted their moves, waiting for game resolution";
//         } else if (state === 2) {
//           status = "Game resolved";
//           liveUpdateMsg = "Game Resolved";
...
//           liveUpdateMsg = "Unknown state, please refresh";
//         }
  
//         console.log(`Setting gameStatus: ${status}`);
//         setGameStatus(status);
//         console.log(`Setting liveUpdate from fetchGameState: ${liveUpdateMsg}`);
//         setLiveUpdate(liveUpdateMsg);
//         setErrorMessage("");
//       } catch (error: unknown) {
...
//           setLiveUpdate(update);
//         } else if (response.move1Submitted) {
//           const update = isPlayer1
//             ? "Player 1 move submitted successfully, waiting for Player 2 to join"
//             : "Player 1 has submitted their move, join to play";
//           console.log(`Setting liveUpdate from checkGameStatus: ${update}`);
...
//         } else {
//           const update = isPlayer1
//             ? "Waiting for your move submission"
//             : "Player 1 is in this game, waiting for you to join";
//           console.log(`Setting liveUpdate from checkGameStatus: ${update}`);
//           setLiveUpdate(update);
//         }
...
//   }, [contract, gameId, socket, currentPlayer, isPlayer1, player2Joined]);
...
//       setPlayer2Joined(true);
//       setLiveUpdate("You joined as Player 2, submit your move");
//       setErrorMessage("");
//     } catch (error: unknown) {
  ...
  useEffect(() => {
    const initContract = async () => {
      try {
        ...
        // Fetch the latest gameCounter from the contract
        const gameCounter = await contractInstance.gameCounter();
        setLatestGameId(gameCounter.toString());
        console.log("Latest gameCounter:", gameCounter.toString());
    ...
    const initialLiveUpdate = searchParams.get("liveUpdate");
    if (gameIdFromUrl) {
      // Validate gameId against latestGameId
      if (latestGameId && parseInt(gameIdFromUrl) > parseInt(latestGameId)) {
        console.warn(`Invalid gameId ${gameIdFromUrl}, resetting to latest: ${latestGameId}`);
        setGameId(latestGameId);
        router.push(`/game?gameId=${latestGameId}`);
      } else {
        setGameId(gameIdFromUrl);
        console.log(`Joining game room: ${gameIdFromUrl}`);
        newSocket.emit("joinGame", gameIdFromUrl);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [getContract, searchParams, latestGameId]);
  ...
  useEffect(() => {
    ...
    const fetchGameState = async () => {
      try {
        ...
        if (state === 0) {
          if (isPlayer1Check) {
            if (game.move1.toString() === "0") { // Explicit check for no move
              status = "Waiting for your move submission";
              liveUpdateMsg = "Waiting for your move submission";
            } else {
              status = game.player2 === ethers.ZeroAddress
                ...
                : "Waiting for Player 2's move";
              liveUpdateMsg = game.player2 === ethers.ZeroAddress
                ? "Your move submitted, waiting for Player 2 to join"
                : "Your move submitted, waiting for Player 2's move";
            }
          } else if (isPlayer2) {
            if (game.move2.toString() === "0") { // Explicit check for no move
              status = "Lobby full, submit your move";
              liveUpdateMsg = "You joined as Player 2, submit your move";
            } else {
              status = "Waiting for resolution";
              liveUpdateMsg = "Your move submitted, waiting for resolution";
            }
          } else {
            status = "Game started, join to play";
            liveUpdateMsg = game.move1.toString() === "0"
              ? "Player 1 is in this game, waiting for you to join"
              : "Player 1 has submitted their move, join to play";
          }
        } else if (state === 1) {
    ...
    socket.on("gameUpdate", (data) => {
      ...
      if (data.gameId === gameId) {
        if (data.action === "joined") {
          ...
          console.log(`Setting liveUpdate: ${update}`);
          setLiveUpdate(update);
        } else if (data.action === "move1Submitted" || data.action === "move2Submitted") {
          // Defer to fetchGameState to confirm move submission
          fetchGameState();
        }
      }
    });
